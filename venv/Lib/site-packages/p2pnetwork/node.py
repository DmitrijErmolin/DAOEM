...
class Node(threading.Thread):
    ...
    def __init__(self, host, port,id, callback=None, server_s=False):
        """Create instance of a Node. If you want to implement the Node functionality with a callback, you should 
           provide a callback method. It is preferred to implement a new node by extending this Node class. 
            host: The host name or ip address that is used to bind the TCP/IP server to.
            port: The port number that is used to bind the TCP/IP server to.
        ...
        self.id = id
        self.connected_id = set()

        # Start the TCP/IP server
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.init_server()
        self.other_validators = []
        self.recieved_data = deque()
        self.timing = {}
        self.recieved_block = deque()
        self.recieved_gen = None
        self.recieved_graph = []
        self.set_validation_user = False
        self.all_conn = False
    ...
    def connect_with_node(self, host, port):
        ...
        for node in self.nodes_outbound:
            if node.host == host and node.port == port:
                self.debug_print("connect_with_node: Already connected with this node.")
                return True
        ...
        try:
            ...
            self.connected_id.add(connected_node_id)
    ...
    def node_message(self, node, data):
        if data.get('command') == "-s":
            self.disconnect_with_node(self.nodes_outbound[self.nodes_inbound.index(node)])
            self.nodes_inbound.remove(node)
            self.connected_id.remove(node.id)
        if data.get('command') == "-c":
            self.connect_with_node(data.get('host'), data.get('port'))
            self.timing[(data.get('host'), data.get('port'))] = data.get('started')
        if data.get('command') == "-send":
            self.recieved_data.append(data.get('data'))
        if data.get('command') == "-send_block":
            if self.server_s or self.set_validation_user is True:
                self.recieved_block.append(data.get('data'))
        if data.get('command') == "-send_gen":
            self.recieved_gen = data.get('data')
        if data.get('command') == "-send_rep":
            self.set_validation_user = True
            for i in data.get('data'):
                if i != self.port:
                    self.other_validators.append(i)
            self.other_validators.append(49001)
        if data.get('command') == "p":
            pass
        if data.get('command') == "-g":
            graph_dict = {}
            for key, value in data.items():
                if key != 'command':
                    graph_dict[key] = value
            update_flag = 0
            for i in range(0, len(self.recieved_graph)):
                if self.recieved_graph[i]['id'] == graph_dict['id']:
                    self.recieved_graph[i]['conn_nodes'] = graph_dict['conn_nodes']
                    update_flag = 1
            if update_flag== 0:
                self.recieved_graph.append(graph_dict)
        if data.get('command') == '-send_base':
            ifo = json.loads(data.get('data'))
            for info in ifo:
                self.base.append([info.get("id"), info.get("ip_address"), info.get("port")])
